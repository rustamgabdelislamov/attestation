...
class OrganizationAdmin(admin.ModelAdmin):
    list_display = ('name', 'contact', 'hierarchy_level', 'debt_to_supplier', 'supplier_link', 'id')  # Поле иерархического уровня вычисляем динамически
    ...
    def supplier_link(self, obj):
        if obj.supplier:
            return format_html('<a href="{}">{}</a>',
                               reverse('admin:electronics_organization_change', args=[obj.supplier.id]),
                               obj.supplier.name)
        return '-'
